...
L'ensemble des applications de E dans F est noté F\expo{E}.
...
\noindent Soient \(f\) : E \(\to\) F et \(g\) : G \(\to\) H deux applications. On dit que \(f\) et \(g\) sont \textbf{égales}\index{égalité d'applications} et on note \vspace{0.1cm} \\
\(f=g\) \ssi : \(\left\{ 
\begin{array}{ll}
...
L'\textbf{application réciproque}\index{application réciproque} de \(f\) est l'application \(f^{-1}\) : F \(\to\) E définie par :\vspace{-0.2cm}
\begin{center}
...
Une \textbf{famille}\index{famille} \(X=\bigl(x_i\bigr)_{i\in I} \) d'éléments x\ind{$i$} d'un ensemble E, indexée par un ensemble I, est une application de I dans E définie par :\vspace{-0.77cm}
\begin{center}\(
\begin{array}{rrcl}
    \hspace{-2cm} X\ : & I & \to & E \\
     & i & \mapsto & x_i
