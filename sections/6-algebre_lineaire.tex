...
Une \textbf{matrice de type n,p à coefficients dans K}\index{matrice de type n,p à coefficients dans K} est une famille \((a_{ij})_{(i,j)\in \llbracket 1,n \rrbracket \times \llbracket 1,p \rrbracket }\,\) d'éléments de\vspace{0.2cm}\\
...
\noindent Une \textbf{matrice ligne}\index{matrice ligne} (resp. \textbf{colonne}\index{matrice colonne}) est une matrice de type 1,p (resp. n,1).\\
Une \textbf{matrice carrée}\index{matrice carrée} d'ordre n est une matrice de type n,n. L'ensemble \(\mathcal{M}_{n,n}(K)\) est noté \(\mathcal{M}_n(K)\).
...
Le \textbf{j\expo{ème} vecteur colonne}\index{j\expo{ème} vecteur colonne} \(C_j\) de A est le vecteur de K\expo{$n$} défini par \(\,C_j=(a_{1j},\cdots,a_{nj}).\)\vspace{0.1cm}\\
La \textbf{i\expo{ème} matrice ligne}\index{i\expo{ème} matrice ligne} \(\mathcal{L} _i\) de A est la matrice ligne de type 1,p définie par \(\, \mathcal{L} _i= \bigl[\, a_{i1} \quad \cdots \quad a_{ip}\, \bigr]\).\vspace{0.1cm}\\
...
Soit \(A\in\mathcal{M}_{n,p}(K)\). Une \textbf{matrice extraite}\index{matrice extraite} de A est une matrice obtenue à partir de A en supprimant des lignes et des colonnes de A.\vspace{0.1cm}\\
\begin{small}
...
Le \textbf{produit matriciel}\index{produit matriciel} \(AB\) de A par B est la matrice de \(\mathcal{M}_{n,p}(K)\) définie par : \(AB=(c_{ij})\) avec \(\displaystyle c_{ij}=\sum_{k=1}^{m}a_{ik}b_{kj}\).
...
\begin{itemize}[leftmargin=0cm,rightmargin=0cm]
    ...
    \item[•] Lorsque \(K=\RR\). On dit que A est \textbf{orthogonale positive}\index{matrice orthogonale positive} \ssi :\(\,A\in O_n(\RR)\,\) et \(\,\det A=1\).\vspace{0.1cm}\\
    \begin{small}
    ...
    \begin{small}
        \underline{\emph{Proposition}} : \((SL_n(\KK),\,\times)\) est un groupe. Il est appelé \textbf{groupe spécial linéaire}\index{groupe spécial linéaire} d'ordre n.
    \end{small}\vspace{0.3cm}

    \item[•] On dit que A est \textbf{nilpotente}\index{matrice nilpotente} \ssi : \(\ \exists\, m\in \NN^* \ \mid \ A^m=0_n\)\vspace{0.2cm}
...
Soient E, F deux espaces vectoriels de dimension \underline{finie} avec dim\,E\(\,=p\!\geq\! 1\), dim\,F\(\,=\!n\geq\!1\) et \(u\in \mathscr{L}(E,F)\). On fixe une base \(\, \mathcal{B}=(e_1,\cdots,e_p)\) de E et une base \(\, \mathcal{C}=(f_1,\cdots,f_n)\) de F.\vspace{0.2cm}\\
...
Autrement dit : \(\alpha_{ij}\) est la i\expo{ème} coordonnée de \(u(e_j)\) dans la base \(\,\mathcal{C}=(f_1,\cdots,f_n)\). L'application linéaire $u$ est donc parfaitement déterminée par la donnée des \(np\) scalaires \((\alpha_{ij})_{\substack{1\leq i\leq n \\ 1\leq j\leq p}}\)\ .\vspace{0.6cm}\\
...
En particulier, on retiendra que la j\expo{ème} colonne de \(M_{\mathcal{B},\,\mathcal{C}}(u)\) est constituée des coordonnées de \(u(e_j)\) dans la base \(\, \mathcal{C}=(f_1,\cdots,f_n)\).\vspace{-0.1cm}
...
Soit \(A\in \mathcal{M}_n(K)\). L'unique endomorphisme \(\,a\in \mathscr{L}(K^n) \) vérifiant \(A=M_{\varepsilon}(a)\), où \(\varepsilon\) est la base canonique de K\expo{$n$}, est appelé \textbf{endomorphisme canoniquement associé à la matrice A}\index{matrices!endomorphisme canoniquement associée}.
...
En particulier, on retiendra que la j\expo{ème} colonne de \(\,M_\mathcal{B}(a_1,\cdots,a_p)\,\) est constituée des coordonnées dans la base \(\, \mathcal{B}\) du vecteur $a_j$.\vspace{1.2cm}\\
...
Soient A et B dans \(\mathcal{M}_{n,p}(K)\). On dit que A est \textbf{équivalente}\index{matrices!équivalence} à B \ssi il existe \(P\in GL_p(K)\) et \(Q\in GL_n(K)\) telles que \(\,Q^{-1}AP=B\).

\vspace{1cm}

Soient A et B dans \(\mathcal{M}_n(K)\). On dit que A est \textbf{semblable}\index{matrices!semblable} à B \ssi il existe \(P\in GL_n(K)\) telle que \(P^{-1}AP=B.\)

\vspace{1.3cm}

\(\left(\mathbf{HP}\right)\,\) Soit X un ensemble quelconque et \(\,f:\mathcal{M}_n(K)\to X\,\) une application. On dit que $f$ est \textbf{invariante par similitude}\index{application invariante par similitude} \ssi deux matrices semblables ont la même image par $f$.\vspace{0.1cm}\\ 
...
\begin{itemize}[leftmargin=0.2cm]
    ...
    Par définition on pose : \(\displaystyle \;\text{I}_A = \bigl\{P\in \poly \ \rvert \ P(A)=0\bigr\}\)\footnote{On l'appelle \textbf{idéal annulateur}\index{matrices!idéal annulateur} de A.}.\vspace{0.1cm}
...
polynôme unitaire \(\,\Pi_A\,\) de $\,\poly\,$ tel que\, I\(_A=\Pi_A\poly.\,\) En cas d'existence, \(\,\Pi_A\,\) est appelé le \textbf{polynôme minimal}\index{matrices!polynôme minimal} de A.
...
\hspace{1cm}i.e. pour \(i\in \llbracket 1,n \rrbracket,\ X_i = [\delta_{i1},\cdots,\delta_{in}]^\text{T}\ \text{ avec }\, \delta _{ij}=1\,\text{ si }\,i=j,\ \; \delta_{ij}=0\,\text{ si }\,i\neq j.\) 
...
On dit que F est \textbf{stable}\index{matrices!s.e.v. stable} par A \ssi \,A(F)$\,\subset\;$F\, c'est-à-dire ssi : \(\forall X\in F,\ AX\in F.\) 
...
On dit que \(\lambda \in K\) est une \textbf{valeur propre}\index{matrices!valeur propre} de A \ssi :\vspace{-0.2cm} \[\exists X\in \mathcal{M}_{n,1}(K)\, \ \rvert \ X\neq 0_{n,1} \  \text{ et }\  AX=\lambda X.\]
...
\(\left(\mathbf{H} \mathbf{P} \right)\) Le \textbf{rayon spectral}\index{matrices!rayon spectral} d'une matrice \(A\in \mathcal{M}_n(\CC)\) est le réel positif : \(\ \ \displaystyle \rho\left(A\right)=\max_{\lambda\,\in\, sp\, A}|\lambda |\,\).
...
Soient \((A,B)\in \mathcal{M}_n(\RR)^2\). On dit que A est \textbf{orthogonalement semblable}\index{matrices!orthogonalement semblable} à B \ssi il existe \(P\in O_n(\RR)\,\) telle que \(\,P^{-1}AP=B.\)
...
\noindent Les éléments de E sont appelés des \textbf{vecteurs}\index{vecteurs} ou des \textbf{points}\index{points}, ceux de K sont appelés des \textbf{scalaires}\index{scalaires}.\vspace{0.1cm} \\
...
Le vecteur nul de E peut être noté 0\ind{E} ou \(\vec{0}\).\vspace{0.3cm}
...
\vspace{-0.1cm}
\begin{itemize}[leftmargin=0.5cm]
    \item[•] Soient\, x\ind{1}, $\cdots$, x\ind{p}\, des vecteurs de E. Une \textbf{combinaison linéaire}\index{combinaison linéaire} de la famille (x\ind{1}, $\cdots$, x\ind{p}) est un vecteur de E de la forme \(\ \alpha_1 \cdot x_1 + \, \cdots \, + \alpha_p \cdot x_p\ \), où \(\ \alpha_1,\cdots, \alpha_p \) sont des scalaires. \vspace{0.1cm}\\
    On note Kx\ind{1} + \(\cdots\) + Kx\ind{p} l'ensemble des combinaisons linéaires de (x\ind{1}, $\cdots$, x\ind{p}).\vspace{0.2cm} \\
    ...
    On pose : E\ind{1}\,+\,E\ind{2} = \(\{ x\in E,\ \exists x_1 \in E_1,\ \exists x_2 \in E_2 \ \mid \ x = x_1 + x_2 \}. \)\vspace{0.1cm}\\
    C'est la \textbf{somme}\index{somme de sous-espaces vectoriels} de E\ind{1} avec E\ind{2} .\vspace{0.1cm} \\
    On dit que la somme E\ind{1}\,+\,E\ind{2} est \textbf{directe}\index{somme directe d'espace vectoriels} \ssi tout vecteur \(\,x \in \) E\ind{1}\,+\,E\ind{2}\ s'écrit de manière \underline{unique} sous la forme  \(x\ind{1} + x\ind{2}\)  avec \(\, x_1 \in  E_1\, \) et \(\, x_2 \in  E_2\ \).\vspace{0.1cm}\\ 
    Pour exprimer que la somme E\ind{1}\,+\,E\ind{2} est directe on la note E\ind{1}\(\,\oplus\,\)E\ind{2} . \vspace{0.3cm}

    \item[•] On dit que les sous-espaces vectoriels E\ind{1} et E\ind{2} sont \textbf{supplémentaires}\index{supplémentaire} dans E \ssi tout vecteur \(x\in\) E s'écrit de manière \underline{unique} sous la forme \(x\ind{1} + x\ind{2}\)  avec \( x_1 \in  E_1 \) et \( x_2 \in  E_2\ \). Autrement dit : E\,=\,E\ind{1}\(\,\oplus\,\)E\ind{2}\vspace{0.1cm} .\vspace{0.1cm}\\
...
\noindent Soient x\ind{1}, $\cdots$, x\ind{p} des vecteurs de E.\vspace{-0.2cm}
\begin{itemize}[leftmargin=1cm]
    \item[•] On dit que (x\ind{1},\(\cdots\), x\ind{p}) est une famille \textbf{libre}\index{famille libre} \ssi :\vspace{0.1cm} \\ \(\ \forall(\alpha_1, \cdots, \alpha_p)\in K^p\ \) on a : \(\, \alpha_1 \cdot x_1 + \ \cdots \ + \alpha_p \cdot x_p=0_E\ \ \Rightarrow \ \ \forall i\in \llbracket 1,p \rrbracket ,\ \alpha_i = 0_K. \) \vspace{0.3cm}

     \item[•] On dit que (x\ind{1},\(\cdots\), x\ind{p}) est une famille \textbf{génératrice}\index{famille génératrice} de E \ssi tout vecteur \(x\in\) E\vspace{0.1cm}\\
     peut s'écrire sous la forme : \(\ x=\alpha_1 \cdot x_1 + \ \cdots \ + \alpha_p \cdot x_p\ \) avec \(\ (\alpha_1, \cdots, \alpha_p)\in K^p\). \vspace{0.3cm}

     \item[•] On dit que (x\ind{1},\(\cdots\), x\ind{p}) est une \textbf{base}\index{base} de E \ssi tout vecteur \(x\in\) E peut s'écrire\vspace{0.1cm}\\
     \emph{de manière unique} sous la forme : \(\ x=\alpha_1 \cdot x_1 + \ \cdots \ + \alpha_p \cdot x_p\ \) avec \(\ (\alpha_1, \cdots, \alpha_p)\in K^p\).\vspace{0.3cm}
      ...
      \item[•] On dit que (x\ind{1},\(\cdots\), x\ind{p}) est une famille \textbf{liée}\index{famille liée} \ssi elle n'est \underline{pas} libre. \vspace{0.1cm}\\
...
Le \textbf{noyau}\index{applications linéaires!noyau} de u : \(\ \ker  u= \{ x\in E \ | \ u(x)=0_F\}. \) \vspace{0.1cm} \\
L'\textbf{image}\index{applications linéaires!image} de u :\hspace{0.34cm} Im\(\; u = \{y\in F,\ \exists\, x\in E \ \mid\ y=u(x) \}. \)

\vspace{1cm}

Supposons que E est de dimension \underline{finie}. Le \textbf{rang de l'application linéaire}\index{applications linéaires!rang} \(u\in \mathscr{L}(E,F)\) est définie par : rg\((f)=\dim(\text{Im}\,f) \).
...
polynôme unitaire \(\,\Pi_u\,\) de $\poly$ tel que\, I\(_u=\Pi_u\poly.\,\) En cas d'existence, \(\,\Pi_u\,\) est appelé le \textbf{polynôme minimal}\index{endomorphismes!polynôme minimal} de $u$.
...
On vérifie que \(u\in \mathscr{L}(F)\). $u_F$ est appelé \textbf{endomorphisme de F induit par u.}\index{endomorphisme induit}
...
On dit que \(\lambda \in K\) est une \textbf{valeur propre}\index{endomorphismes!valeur propre} de $u$ \ssi : \(\exists\, x\in E \ \rvert \ x\neq 0_E \, \text{ et }\,  u(x)=\lambda x.\)\\
...
\( \varphi(x_1,\cdots, x_p)=0_K  \) pour tout p-uplet \((x_1,\cdots, x_p)\) de E\expo{p} dont au moins deux vecteurs sont égaux.\vspace{0.5cm}\\
...
\textbf{déterminant de la famille}\index{déterminant d'une famille} de vecteurs \((x_1,\cdots,x_n)\) dans la base \(\mathcal{B}\).
...
Le scalaire \(\,A_{ij}=(-1)^{i+j}\det M_{ij}\, \) est appelé \textbf{cofacteur}\index{cofacteur} de \(\alpha_{ij} \) dans la matrice \(M\).\vspace{0.5cm}
