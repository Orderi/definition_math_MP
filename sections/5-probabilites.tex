...
Une \textbf{tribu}\index{tribu} sur l'ensemble \(\Omega\) est un ensemble \(\mathcal{A}\) de parties de \(\Omega\) vérifiant :
...
Une \textbf{variable aléatoires discrète}\index{variable aléatoires discrète} (abrégée en VAD) sur \(\bigl(\Omega,\,\mathcal{A}\bigr)\) à valeurs dans E est une application \(\,X:\Omega \to \text{E}\,\) qui vérifie les propriétés :\vspace{0.1cm}
\begin{enumerate}[leftmargin=2cm]
...
On dit que X et Y sont \textbf{indépendantes}\index{VAD indépendantes} pour \(\,\PP\,\), et on note \(X\perp Y\) ou \(X\indep Y\), \ssi :\vspace{-0.25cm}
...
X et Y sont dites \textbf{indépendantes et identiquement distribuées}\index{i.i.d}\index{VADR indépendantes et identiquement distribuées} (i.i.d) pour une probabilité $\PP$ si elles sont indépendantes et de même loi. (i.e : \(X\sim Y\) \underline{et} \(X\indep Y\))
...
La \textbf{covariance}\index{covariance} de deux variables aléatoires\footnote{Variables aléatoires \underline{réelles}. On a défini $\displaystyle L^p$ avec $p\geq 2$ seulement dans le cas réel.} X et Y de \(\,L^2\,\) est le réel cov(X,Y) défini par :\vspace{0.1cm}
...
Soit \(X\in L^2\).\\
La \textbf{variance}\index{variance} de X est le réel prositif \(\,\VV(X)\,\) défini par \(\,\VV(X)=\EE\bigl(C_{_X}^{\,2}\bigr)\;\) où \(\,\,C_{_X}\!=X-\EE(X)\).\vspace{0.15cm}\\
L'\textbf{écart-type}\index{écart-type} de X est le réel positif \(\,\sigma(X)\,\) défini par \(\sigma(X)=\sqrt{\VV(X)}\).
...
La variable aléatoire X est dite \textbf{réduite}\index{VAD réduite} \ssi \(\,\VV(X)=1.\)
